    ...
    function renderSearchResults(results) {
                ...
                list.innerHTML += `
                    <div style="background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.06); border-radius:12px; padding:16px;">
                        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; flex-wrap:wrap; gap:8px;">
                            ...
                            <a href="${demandUrl}" style="font-size:11px; font-weight:800; background:rgba(239,68,68,0.15); color:#f87171; padding:2px 8px; border-radius:6px; text-decoration:none; cursor:pointer; transition:all 0.2s;" onmouseover="this.style.background='rgba(239,68,68,0.25)'" onmouseout="this.style.background='rgba(239,68,68,0.15)'">
        ...
        if (results.workers && results.workers.length > 0) {
            ...
            results.workers.forEach(w => {
                ...
                const profileUrl = `/ledger/?action=profile&target_id=${targetId}`;
                const paymentUrl = `/ledger/?action=payment&target_id=${targetId}`;
