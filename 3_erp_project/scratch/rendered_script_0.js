    ...
    async function saveAttendanceState(workerId, status, date, otHours) {
        ...
        formData.append('csrfmiddlewaretoken', 'geFSztzYwWK2aVyM6qkYVTYT71z69c0BfQgT9HqiqZZDeyTfOidpUfOulyq40x8u');
    ...
    async function saveAllWorkerRates() {
        ...
        try {
            const res = await fetch('/api/allocation/add/', {
                method: 'POST',
                body: fd,
                headers: {'X-CSRFToken': 'geFSztzYwWK2aVyM6qkYVTYT71z69c0BfQgT9HqiqZZDeyTfOidpUfOulyq40x8u'}
            });
            const data = await res.json();
            if (data.status === 'success') {
                resetRateRowsContainer();
