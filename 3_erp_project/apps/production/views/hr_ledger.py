...
def labor_ledger(request):
    ...
    # Bulk pre-fetch all attendance for internal workers
    month_attendance = list(Attendance.objects.filter(
        worker__in=internal_workers,
        date__gte=month_start,
        date__lte=month_end
    ))
    att_by_worker_date = {(a.worker_id, a.date): a for a in month_attendance}
    att_by_worker = defaultdict(list)
    for a in month_attendance:
        att_by_worker[a.worker_id].append(a)

    # Bulk pre-fetch casting transactions for internal workers
    ...
    # Bulk pre-fetch all payments for internal workers
    all_staff_payments = list(LaborPayment.objects.filter(worker__in=internal_workers))
    pmts_by_worker = defaultdict(list)
    for p in all_staff_payments:
        pmts_by_worker[p.worker_id].append(p)

    # Bulk pre-fetch active loans
    staff_active_loans = {l.worker_id: l for l in Loan.objects.filter(worker__in=internal_workers, is_active=True)}

    # Bulk pre-fetch settlement locks for current and previous month
    from apps.authentication.models import MonthlySettlementLock
    staff_locks = {sl.worker_id: sl for sl in MonthlySettlementLock.objects.filter(worker__in=internal_workers, month_str=month_str)}
    prev_month_str = (month_start - timedelta(days=1)).strftime('%Y-%m')
    prev_staff_locks = {sl.worker_id: sl for sl in MonthlySettlementLock.objects.filter(worker__in=internal_workers, month_str=prev_month_str)}
    ...
    for w in internal_workers:
        ...
        # Calculate paid holiday pay for daily workers
        ...
        # Payments & Repayments this month from pre-fetched list
        ...
        # Opening balance / Carry forward from prior month if unpaid
        prior_unpaid_bal = 0.0
        if prev_month_str >= '2026-05':
            p_lock = prev_staff_locks.get(w.id)
            if p_lock and p_lock.is_locked:
                ...
                if r_prev.get('balance', 0) > 0 and r_prev.get('status_code') != 'FULLY_PAID':
                    prior_unpaid_bal = max(0.0, float(r_prev['balance']))
        ...
        active_loan = staff_active_loans.get(w.id)
        ...
        lock_obj = staff_locks.get(w.id)
        if not is_current_month and status_code == 'FULLY_PAID' and (not lock_obj or not lock_obj.is_locked):
            from apps.authentication.models import MonthlySettlementLock
            lock_obj, _ = MonthlySettlementLock.objects.update_or_create(
                worker=w,
                month_str=month_str,
                defaults={
                    'is_locked': True,
    ...
    # Bulk pre-fetch all allocations for job workers
    jw_allocations = list(ItemWorkerAllocation.objects.filter(worker__in=job_workers).select_related('item'))
    jw_alloc_map = {(a.worker_id, a.item_id): float(a.rate_per_piece) for a in jw_allocations}
    jw_alloc_code_map = {(a.worker_id, a.item.code): float(a.rate_per_piece) for a in jw_allocations}

    # Pre-fetch all compositions for SET items
    ...
    # Bulk pre-fetch all StockTransactions for job workers
    all_jw_txs = list(StockTransaction.objects.filter(
        worker__in=job_workers
    ).select_related('item'))
    jw_txs_by_worker = defaultdict(list)
    for tx in all_jw_txs:
        jw_txs_by_worker[tx.worker_id].append(tx)

    # Bulk pre-fetch all payments for job workers
    all_jw_pmts = list(LaborPayment.objects.filter(worker__in=job_workers))
    jw_pmts_by_worker = defaultdict(list)
    for p in all_jw_pmts:
        jw_pmts_by_worker[p.worker_id].append(p)

    jw_active_loans = {l.worker_id: l for l in Loan.objects.filter(worker__in=job_workers, is_active=True)}

    # Fast in-memory month-by-month calculation for Job Workers (Zero recursive SQL queries)
    def calc_jw_month_metrics(jw_id, m_str, m_open_bal):
        ...
        if jw_obj and jw_obj.casting_rate_per_kg > 0:
            ...
            c_pay = tot_c_wt * float(jw_obj.casting_rate_per_kg)
            t_earned += c_pay

        w_pmts = jw_pmts_by_worker[jw_id]
        pmts_m = [
            p for p in w_pmts
        ...
        return {
            ...
            'raw_total_earned': t_earned,
            'raw_total_paid': t_paid,
    ...
    for jw in job_workers:
        ...
        final_m_res['jw'] = jw
        final_m_res['active_loan'] = jw_active_loans.get(jw.id)
    ...
    # Dynamically generate available months from current month down to start of 2026
    available_months = []
    start_y = 2026
    curr_y = now_dt.year
    curr_m = now_dt.month
    for y in range(curr_y, start_y - 1, -1):
        max_m = curr_m if y == curr_y else 12
        min_m = 1
        for m in range(max_m, min_m - 1, -1):
            m_date = datetime(y, m, 1)
            available_months.append({
                'val': m_date.strftime('%Y-%m'),
                'name': m_date.strftime('%B %Y'),
            })

    # Ensure selected month is in available_months
    if month_str not in [m['val'] for m in available_months]:
        try:
            m_custom = datetime.strptime(month_str, '%Y-%m')
            available_months.insert(0, {
                'val': month_str,
                'name': m_custom.strftime('%B %Y')
            })
        except Exception:
            pass
    ...
    context = {
        ...
        'active_tab': request.GET.get('tab', 'jw' if request.GET.get('target_id', '').startswith('jw_') else 'staff'),
        'active_drawer': 'jw_profile' if (request.GET.get('action') == 'profile' and request.GET.get('target_id', '').startswith('jw_')) else ('worker_profile' if (request.GET.get('action') == 'profile' and request.GET.get('target_id', '').startswith('w_')) else ('payment' if request.GET.get('action') == 'payment' else request.GET.get('drawer', ''))),
        'drawer_worker_id': request.GET.get('target_id', '').replace('w_', '') if (request.GET.get('action') == 'profile' and request.GET.get('target_id', '').startswith('w_')) else request.GET.get('worker_id', ''),
        'drawer_jw_id': request.GET.get('target_id', '').replace('jw_', '') if (request.GET.get('action') == 'profile' and request.GET.get('target_id', '').startswith('jw_')) else request.GET.get('jw_id', ''),
        'drawer_target_id': request.GET.get('target_id', ''),
        'company1': company1,
        'company2': company2,
    }
...
def get_worker_report_data(worker, month_str):
    ...
    if prev_month_str >= '2026-05':
        from apps.authentication.models import MonthlySettlementLock
        prev_lock = MonthlySettlementLock.objects.filter(worker=worker, month_str=prev_month_str, is_locked=True).first()
        if prev_lock:
            prior_earned = round(prev_lock.locked_earnings)
        else:
            r_prev = get_worker_report_data(worker, prev_month_str)
            prior_due = round(r_prev['total_earned'])
            if r_prev['status_code'] == 'FULLY_PAID':
                prior_earned = max(prior_due, round(r_prev['total_paid']))
            else:
                prior_earned = prior_due
    settlement_loan_deductions = sum(p.amount for p in payments.filter(payment_type='LOAN_REPAYMENT') if (p.notes and ('deducted from settlement' in p.notes.lower() or 'loan recovery' in p.notes.lower())))
    total_discharged = total_paid_regular + settlement_loan_deductions
    ...
    if lock_obj and lock_obj.is_locked:
        ...
        if lock_obj.locked_earnings > 0 or lock_obj.locked_paid > 0:
            ...
            total_paid_regular = lock_obj.locked_paid
...
def get_job_worker_report_data(jw, month_str):
    ...
    for entry in item_ledger:
        # Smart Filter: Include item if it has active work (received > 0), material issued (issued > 0), or positive stock in hand (balance > 0)
        has_monthly_activity = (entry['total_received'] > 0 or entry['total_issued'] > 0 or entry['balance'] > 0)
        if not has_monthly_activity and not is_all_time:
            continue
        ...
        matrix_items.append({
            ...
            'lot_size': getattr(entry['item'], 'lot_size', 1) or 1,
            'rate': entry['rate'],
    ...
    if num_days > 0:
        for d in range(1, num_days + 1):
            ...
            day_casting_wt = daily_casting.get(dt, 0.0)
            day_casting_earn = day_casting_wt * float(jw.casting_rate_per_kg) if jw.casting_rate_per_kg > 0 else 0.0
            ...
            matrix_days.append({
                ...
                'advance_paid': day_pmt,
                'casting_weight': day_casting_wt,
                'casting_earned': day_casting_earn,
                'has_activity': row_has_activity or day_pmt > 0 or day_casting_wt > 0
            })
    ...
    if prev_month_str >= '2026-05':
        ...
        if r7_jw['status_code'] == 'FULLY_PAID':
            ...
            financial_opening_bal = max(0.0, float(r7_jw.get('balance', 0.0)))

    cum_gross_due = max(0.0, financial_opening_bal + total_earned)
    raw_month_pmts = sum(p.amount for p in payments.exclude(payment_type__in=['NEW_LOAN', 'LOAN_REPAYMENT']))
    settlement_loan_deductions = sum(p.amount for p in payments.filter(payment_type='LOAN_REPAYMENT') if (p.notes and ('deducted from settlement' in p.notes.lower() or 'loan recovery' in p.notes.lower())))
    advance_surplus = max(0.0, (raw_month_pmts + settlement_loan_deductions) - cum_gross_due)
    total_paid = raw_month_pmts
    total_discharged = total_paid + settlement_loan_deductions
    net_balance = max(0.0, round(cum_gross_due) - round(total_discharged))
    if abs(net_balance) < 1.0:
        net_balance = 0.0
    if total_earned == 0 and financial_opening_bal <= 0.99 and raw_month_pmts == 0 and total_repaid == 0:
        status_code = 'NO_ACTIVITY'
    elif advance_surplus > 0:
        status_code = 'ADVANCE_PAID'
    elif net_balance <= 0 and (total_earned > 0 or financial_opening_bal > 0.99 or raw_month_pmts > 0 or settlement_loan_deductions > 0):
        status_code = 'FULLY_PAID'
    ...
    return {
        ...
        'total_repaid': total_repaid,
        'total_discharged': total_discharged,
        'advance_surplus': advance_surplus,
...
def job_worker_monthly_report(request, jw_id):
    ...
    all_company_items = list(Item.objects.filter(active=True).order_by('code'))
    context.update({
        'all_jws': all_jws,
        'prev_jw_id': prev_jw_id,
        'next_jw_id': next_jw_id,
        'all_company_items': all_company_items,
    })
...
def toggle_settlement_lock(request):
    ...
    if action == 'unlock':
        ...
        lock_obj.locked_paid = float(report['total_paid'])
        lock_obj.locked_by = request.user
        lock_obj.save()
...
def attendance_view(request):
    ...
    # Monthly Attendance Sheet Matrix data
    month_raw = request.GET.get('month', '').strip()
    month_dt = None
    if month_raw:
        for fmt in ('%Y-%m', '%Y-%m-%d', '%Y/%m', '%Y/%m/%d'):
    ...
    for w in workers:
        ...
        for d in days_range:
            ...
            record = att_by_worker_date.get((w.id, dt))
            h_info = get_holiday_for_date(dt, w.company, month_holidays)
            date_str = dt.strftime('%Y-%m-%d')
    ...
    now_dt = timezone.now().date()
    available_months = []
    for y in range(now_dt.year, 2025, -1):
        max_m = now_dt.month if y == now_dt.year else 12
